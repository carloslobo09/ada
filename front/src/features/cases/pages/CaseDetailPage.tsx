...
import { CaseClienteResultPanel } from "@/features/cases/components/CaseClienteResultPanel";
...
export function CaseDetailPage(): ReactNode {
  ...
  return isCliente ? <ClienteView /> : <EntrenadorView />;
}

function ClienteView(): ReactNode {
  const { caseId } = useParams<{ caseId: string }>();
  const { data, isLoading, isError, error } = useGetCaseCliente(caseId);
  ...
  return (
    <div className="space-y-6">
      ...
      <CaseClienteResultPanel caso={data} />
    </div>
  );
}
